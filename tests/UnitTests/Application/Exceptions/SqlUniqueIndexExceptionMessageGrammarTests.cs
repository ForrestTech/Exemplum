...
public class SqlUniqueIndexExceptionMessageGrammarTests
{
    ...
    [InlineData("Wrong")]
    [InlineData(
        "Cannot insert duplicate key row in object 'noperiod' with unique index 'nounderscores'. The duplicate key value is (Shopping).")]
    [InlineData(
        "Cannot insert duplicate key row in wrong 'dbo.TodoLists' with unique index 'IX_TodoLists_Title'. The duplicate key value is (Shopping).")]
    [InlineData(
        "Cannot insert duplicate key row in object 'dbo.TodoLists' with wrong index 'IX_TodoLists_Title'. The duplicate key value is (Shopping).")]
    [InlineData(
        "Cannot insert duplicate key row in object 'dbo.TodoLists' with wrong index 'IX_TodoLists_Title'. The duplicate key value wrong (Shopping).")]
